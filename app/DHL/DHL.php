    ...
    public function getShipmentAsPdf($shipmentTrackingNumber, array $queryParams = [])
    {
        ...
        if (isset($response->documents)) {
            ...
            foreach ($response->documents as $key => $document) {
                $pdf = fopen ('../../pdf'.$key.'.pdf','w');
                fwrite ($pdf,base64_decode($document->content));
                fclose ($pdf);
                array_push($pdfs,'../../pdf'.$key.'.pdf');
            }
            foreach ($pdfs as $pdf) {
                $merge->add($pdf);
                unlink($pdf);
